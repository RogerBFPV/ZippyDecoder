...
//#define FILTER_IMPL FILTER_NONE
...
struct Settings
{
    ...
    rssi_t volatile triggerThreshold = 25;
    rssi_t volatile calibrationThreshold = 80;
    rssi_t volatile calibrationOffset = 10;
};
...
struct State
{
    ...
    rssi_t volatile rssiTrigger = 0; // above this and crossing has begun 
    rssi_t volatile nodeRssiPeak = 0; // peak smoothed rssi seen since the node frequency was set
    mtime_t nodeRssiPeakTimestamp = 0; // time of this peak 
...
struct LastPass
{
    rssi_t volatile nodeRssiPeak = 0;
    mtime_t volatile nodeRssiLaptime = 0;
    mtime_t volatile nodeRssiPeakTimestamp = 0;
...
public:
    ...
    rssi_t getTriggerThreshold() { return settings.triggerThreshold; }
    void setTriggerThreshold(rssi_t val) { settings.triggerThreshold = val; }

    rssi_t getCalibrationThreshold() { return settings.calibrationThreshold; }
    void setCalibrationThreshold(rssi_t val) { settings.calibrationThreshold = val; }

    rssi_t getCalibrationOffset() { return settings.calibrationOffset; }
    void setCalibrationOffset(rssi_t val) { settings.calibrationOffset = val; }
