    ...
    def read_block(self, interface, command, size, max_retries=MAX_RETRY_COUNT):
        ...
        while success is False and retry_count <= max_retries:
            try:
                ...
                if data:
                    ...
                    if retry_count <= max_retries:
                        if retry_count > 1:  # don't log the occasional single retry
                            interface.log('Retry (checksum) in read_block:  addr={0} cmd={1} size={2} retry={3} ts={4}'.format(self.i2c_addr, hex(command), size, retry_count, self.i2c_helper.i2c_timestamp))
                    else:
                        interface.log('Retry (checksum) limit reached in read_block:  addr={0} cmd={1} size={2} retry={3} ts={4}'.format(self.i2c_addr, hex(command), size, retry_count, self.i2c_helper.i2c_timestamp))
                ...
                if retry_count <= max_retries:
                    if retry_count > 1:  # don't log the occasional single retry
                        interface.log('Retry (IOError) in read_block:  addr={0} cmd={1} size={2} retry={3} ts={4}'.format(self.i2c_addr, hex(command), size, retry_count, self.i2c_helper.i2c_timestamp))
                else:
                    interface.log('Retry (IOError) limit reached in read_block:  addr={0} cmd={1} size={2} retry={3} ts={4}'.format(self.i2c_addr, hex(command), size, retry_count, self.i2c_helper.i2c_timestamp))
                self.inc_read_error_count(interface)
        return data
...
def discover(idxOffset, i2c_helper, isS32BPillFlag=False, *args, **kwargs):
    print("in I2C nodes discover...")
    if not isS32BPillFlag:
        logger.info("Searching for I2C nodes...")
        print("Searching for I2C nodes...")
    nodes = []
